...
| 섹션 | 역할 |
|------|------|
| `## Todo` | 미완료 할일 (최상단, 구 Todo.md 역할). 체크박스 `- [ ]`로 표기 |
| `## YYYY-MM-DD` | 날짜별 완료 이력 (최신이 위, 구 change_notes.md 역할) |

### 작성 규칙

1. **할일 추가**: `Todo`에 `- [ ]` 체크박스로 추가 (우선순위순)
2. **작업 완료 시**: `Todo`에서 항목을 제거하고, 해당 날짜 섹션에 작업 내용을 기록
   - 신규 기록은 `### Added` / `### Changed` / `### Fixed`로 분류 (통합 이전 이력은 원문 유지)
3. **새 날짜 작업**: `Todo` 바로 아래에 새 날짜 섹션 생성 (역순 유지)
4. **진행 중 표기**: `Todo` 항목에 `(진행 중: N/M단계)` 부기
